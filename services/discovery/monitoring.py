...
def get_latest_observation_for_mission(
    ...
    logger.info(f"Looking for monitoring observation between {target_start.isoformat()} and {target_end.isoformat()}")
    ...
    logger.info(f"Selected observation {selected.observation_id} acquired at {selected.scene.acquired_at}")
    return selected
