...
class BhoonidhiAdapter(AbstractProvider):
    ...
    def __init__(self):
        ...
        self.redis_client = redis.from_url(config.redis_url.get_secret_value(), decode_responses=True)
    ...
    def _get_auth_token(self, context: dict = None) -> str:
        ...
        with tracer.start_as_current_span("bhoonidhi_auth") as span:
            ...
            try:
                ...
                raise RuntimeError("Failed to acquire auth lock. Another worker might be authenticating.")
    ...
    def search(
        ...
        **kwargs
        ...
        with tracer.start_as_current_span("bhoonidhi_search") as span:
            ...
            payload = {
                ...
                "filter": {
                    ...
                    "args": [
                        {
                            "op": "s_intersects",
                            "args": [{"property": "geometry"}, polygon]
                        },
                        {
                            "op": "t_intersects",
                            "args": [{"property": "datetime"}, [start_date.isoformat() + "Z", end_date.isoformat() + "Z"]]
                        },
                        {
                            "op": "<=",
            ...
            resp = self.session.post(f"{self.base_url}/data/search", headers=headers, json=payload, timeout=self.timeout)
            resp.raise_for_status()
    ...
    def get_asset(self, item_id: str, asset_key: str, context: dict = None) -> Optional[str]:
        ...
        """
        from services.eo_data.telemetry import tracer, inject_context_to_span, asset_download_failure_total
        ...
        with tracer.start_as_current_span("bhoonidhi_download") as span:
            ...
            try:
                # We enforce max 3 concurrent downloads globally via ingestion queue (managed higher up)
                with self.session.get(download_url, headers=headers, stream=True, timeout=(5.0, 60.0)) as r:
                    r.raise_for_status()
                    # Use boto3 upload_fileobj which efficiently handles streaming without exhausting memory
