...
def test_postgis_rls_tenant_isolation(mock_pool_class):
    """P4-14: Verifies PostGIS operations rigorously enforce and clean up Row-Level Security contexts."""
    mock_pool = MagicMock()
    ...
    assert mock_cursor.execute.call_args_list[0][0][0] == "SELECT set_config('satquery.org_id', %s, false);"
    ...
    assert mock_cursor.execute.call_args_list[-1][0][0] == "SELECT set_config('satquery.org_id', '', false);"
