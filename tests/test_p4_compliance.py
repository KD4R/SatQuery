...
Each issue gets exactly 4 tests: valid path, invalid/error path, service boundary, schema compatibility.
...
def test_observation_normalization_pipeline_valid():
    """SearchService.search_observations normalises a Bhoonidhi feature into an Observation with all required fields."""
...
def test_spatial_temporal_observation_search_valid():
    ...
    with patch("services.eo_data.implementation.redis_client", None):
        results = svc.search_observations({}, datetime.now(timezone.utc), datetime.now(timezone.utc), {})
    assert results == []
...
def test_spatial_temporal_observation_search_api_valid():
    """POST /observations/search with valid auth header and body returns 200 or 500 (not 404/405)."""
    body = {
        "polygon": {"type": "Polygon", "coordinates": [[[79.8, 15.1],[80.2, 15.1],[80.2, 15.5],[79.8, 15.5],[79.8, 15.1]]]},
        "start_date": "2026-09-01T00:00:00Z",
        "end_date": "2026-09-10T00:00:00Z",
    }
    with patch("services.eo_data.implementation.SearchService.search_observations", return_value=[]):
        resp = client_eo.post(
...
def test_spatial_temporal_observation_search_api_invalid_input():
    ...
    resp = client_eo.post(
        "/api/v1/observations/search",
        json={"polygon": {}, "start_date": "2026-01-01T00:00:00Z", "end_date": "2026-01-10T00:00:00Z"},
    )
    assert resp.status_code == 401

def test_p4_07_service_boundary():
    """POST /observations/search with empty body returns 422 (validation error), not 500."""
    resp = client_eo.post("/api/v1/observations/search", json={}, headers={"organization-id": "org-test"})
...
def test_asset_resolver_valid():
    ...
    with patch("services.eo_data.implementation.BhoonidhiAdapter.get_asset", return_value="s3://bucket/key.tif"):
        resp = client_eo.post(
...
def test_p4_08_schema_compatibility():
    ...
    with patch("services.eo_data.implementation.BhoonidhiAdapter.get_asset", return_value="s3://bucket/k.tif"):
        resp = client_eo.post(
            "/api/v1/assets/resolve",
...
def test_aoi_clipping_and_windowed_processing_invalid_input():
    ...
    from packages.geo.clipping import clip_raster_to_aoi
    aoi = {"type": "Polygon", "coordinates": [[[79.8, 15.1],[80.2, 15.1],[80.2, 15.5],[79.8, 15.5],[79.8, 15.1]]]}
    with pytest.raises(Exception):
...
def test_p4_12_schema_compatibility():
    ...
    real_meta = {"driver": "GTiff", "count": 1, "dtype": "uint8", "crs": "EPSG:32644", "transform": None}
    mock_src = MagicMock()
...
def test_cog_generation_and_overviews_valid():
    """generate_cog calls gdaladdo (overviews) then gdal_translate (COG) — verified by subprocess mock."""
...
def test_p4_13_schema_compatibility():
    ...
    from packages.geo.cog import generate_cog
    with patch("os.path.exists", return_value=True):
        with patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, "gdaladdo", stderr="error")):
...
def test_p4_15_schema_compatibility():
    """geo/api.py imports TilerFactory from titiler.core inside a try/except — not a hard dependency."""
...
def test_monitoring_observation_selection_support_valid():
    """get_latest_cloud_free_observation returns the cloud-free scene from the monitoring fixture."""
    from services.eo_data.implementation import SearchService
    svc = SearchService.__new__(SearchService)
    svc.bhoonidhi = MagicMock()
...
def test_p4_18_schema_compatibility():
    """get_latest_cloud_free_observation returns None (not exception) when provider returns empty."""
    from services.eo_data.implementation import SearchService
    svc = SearchService.__new__(SearchService)
